      ...
      <form method="post" id="create_form">
         ...
         <div class="modal-content">
            <div class="modal-header ">
               <h4 class="modal-title">Tambah Kategori / OU</h4>
            ...
            <div class="modal-body">
               <div class="form-group row">
                  ...
                  <div class="col-sm-8">
                     <input type="text" name="name" class="form-control" id="name" required placeholder="namaou">
                  </div>
               </div>
               <div class="form-group row">
                  <label for="base_dn" class="col-sm-4 col-form-label">Base DN</label>
                  <div class="col-sm-8">
                     <input type="text" name="base_dn" class="form-control" id="base_dn" required value="dc=stmikelrahma,dc=ac,dc=id">
                  </div>
               </div>
               <div class="form-group row">
...
<script type="text/javascript">
   ...
   $(document).on('click', '.edit', function() {
      var id = $(this).data('id');
      $.ajax({
         url: '<?= base_url() ?>/admin/ctgeditfetch',
         method: "POST",
         data: {
            id: id
         },
         ...
         success: function(data) {
            ...
            $('#editModal .modal-title').text('Edit Kategori');
            let dom = data.domain;
            let i = '';
            let z = '';
            let selection;
            let disabled;
            data.protected === 't' ? readonly = 'readonly' : readonly = '';
            // console.log(data);
            ...
            $('#editModal #nama_kategori').html('<div class="form-group row"> <label for="name" class="col-sm-4 col-form-label">Nama Kategori</label> <div class="col-sm-8"> <input type="text" name="name" class="form-control" id="name" value="' + data.name + '" ' + readonly + '> </div></div>');
            $('#editModal #base_dn').html('<div class="form-group row"> <label for="base_dn" class="col-sm-4 col-form-label">Base DN</label> <div class="col-sm-8"> <input type="text" name="base_dn" class="form-control" id="base_dn" value="' + data.base_dn + '"' + readonly + '> </div></div>');
   ...
   $('#aksi_form').on('submit', function(event) {
      ...
      $.ajax({
         url: "<?= base_url() ?>/admin/ctgeditaction",
         method: "POST",
         data: $(this).serialize(),
         dataType: "JSON",
         beforeSend: function() {
